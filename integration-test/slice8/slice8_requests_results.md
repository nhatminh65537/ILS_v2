...
| I-4.1 | PASS | Member1 sees only own sessions | HTTP 200 with own session ids | HTTP 200, count=3 |
| I-4.2 | PASS | Member1 cannot revoke member2 session | HTTP 404 | HTTP 404 |
| II-1.1 | PASS | GET /api/users/me/profile/ | HTTP 200 | HTTP 200 |
| II-1.2 | FAIL | Me profile has expected username | username=member1 | username=member1_new |
...
| II-3.3 | FAIL | PATCH settings invalid language | HTTP 400 | HTTP 200 |
| II-3.4 | FAIL | PATCH settings invalid theme | HTTP 400 | HTTP 200 |
...
| II-4.5 | FAIL | Restore username to member1 | HTTP 200 and username restored | HTTP 400, username=['A user with that username already exists.'] |
...
| III-1.6 | PASS | Disabled user public profile behavior | HTTP 200 or 404 | HTTP 200 |
| IV-1.1 | PASS | GET /api/users/me/activity/ | HTTP 200 | HTTP 200 |
| IV-1.2 | PASS | Member1 has 6 seeded events | 6 events | count=6 |
...
| IV-2.2 | PASS | Public activity equals me activity count | same event count | public=6, me=6 |
...
| V-1.3 | PASS | Admin list includes seeded users | >=6 users | count=11 |
| V-2.1 | PASS | Admin filter is_active=false | HTTP 200 | HTTP 200 |
| V-2.2 | PASS | Inactive filter returns disableduser | all users inactive | items=1 |
...
| V-4.4 | FAIL | Admin create duplicate email | HTTP 400 | HTTP 201 |
...
| VI-1.2 | PASS | Member1 has at least 1 active session | session count >= 1 | count=4 |
