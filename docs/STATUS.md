...
| Slice 2 / Authz bitmap fix (2026-06-01) | **F38 (CRITICAL):** `HasJWTPermission` đọc claims qua `isinstance(request.auth, dict)` nhưng `request.auth` là `AccessToken` → bitmap bị bỏ qua, authz rơi xuống fallback role-name → custom role/`system.*` perms vô tác dụng khi authz enabled. Sửa: bitmap-only fail-closed, bỏ fallback + `is_superuser`; refactor test fixtures sang JWT thật + `test_authz_bitmap.py`. Phát hiện qua Integration Test Pass 1 nhóm B [B-03]. Report: `docs/reports/2026-06-01_rbac-bitmap-fix-and-followups.md`. |
| Slice 6 / Challenge file-explorer refactor (2026-06-04) | **F46:** Tái tổ chức Challenge thành **file-explorer mở sâu** (parity với Learn), giữ mô hình cây toàn cục (không migration, không đụng `BaseNode`). (BE) sort folder-first + title A→Z; `create_challenge_node_atomic` (item → draft Challenge + node); `move_challenge_node_bulk` (hết N+1) + cycle/depth guard; endpoint explorer `GET /api/challenge/nodes/{explorer,/{id}/explorer}/` (folders+items+breadcrumb+`is_solved`); filter tag-AND (`?tags=`) + `?solved=` + `LimitOffsetPagination` + `is_solved` list; seed `challenge.max_tree_depth`. (FE) admin file-explorer (breadcrumb/back, URL `?folder=`, tile + checkbox bulk delete ConfirmDialog, tạo folder/challenge atomic→editor, Move dialog); trang taxonomy riêng `challenges/{categories,tags}`; gỡ tree tab/2 taxonomy dialog/`window.confirm,prompt`/route `new`. User catalog mode-aware (explorer ↔ flat-search BE) + highlight đã-giải. **Reorder kéo-thả CỐ TÌNH bỏ** (CTF độc lập). Full BE suite exit 0 (+8 test); FE tsc/build exit 0. Report: `docs/reports/2026-06-04_challenge-file-explorer-refactor.md`. |
| Slice 7 / Quiz file-explorer + taxonomy parity (2026-06-04) | Đưa Quiz lên ngang Challenge (theo PRD FR-QUIZ-02 "giống challenge tree pattern" + FR-QUIZ-08). (BE) `QuizService` thêm `create_quiz_node_atomic` (item → draft Quiz + node, không slug — dùng `id`), `move_quiz_node_bulk` (hết N+1) + cycle/depth, `list_explorer_children` folder-first, `filter_visible_quizzes_flat` (search/category/tag-AND/solved), `upsert_quiz_tags`; serializers `QuizNodeWriteSerializer`/`QuizExplorerSerializer` + `category_id`/`tag_ids` trên detail; viewsets `QuizCategoryViewSet`/`QuizTagViewSet` + viết lại `QuizNodeViewSet` (explorer root/folder); URLs `quiz/{categories,tags,nodes/explorer}`; seed `quiz.max_tree_depth`. (FE) admin **file-explorer dạng TABLE** (folder rows trước, breadcrumb/back, `?folder=`, bulk move/delete, tạo folder/quiz atomic→editor); editor 1 trang **Tabs** (Metadata + category/tag | Questions inline) — bỏ route `/questions` & `/new`; trang taxonomy `quizzes/{categories,tags}`; user catalog mode-aware (explorer ↔ flat-search) + filter sidebar (search/category/tag/solved) như challenge. Quiz tests pass (+8 test mới); FE tsc/eslint exit 0. Report: `docs/reports/2026-06-04_quiz-explorer-parity.md`. |
| Slice 5 / Learn admin tree + catalog filter (2026-06-03) | **Integration Test Pass 1 nhóm D + phụ (_-01.._-13, D-04):** thay native browser dialog bằng `ConfirmDialog` app; thêm xóa vĩnh viễn (purge) course bên cạnh archive (`?mode=`); sửa bug folder rỗng sau create (`refreshTree` reload children mọi folder expand); **redesign cây thư mục bằng drag-and-drop (@dnd-kit)** thay reorder hỏng — endpoint mới `POST .../nodes/reorder/` reindex `position=0..n` qua `ordered_ids`; sort folder-first (`order_by('is_item','position','id')`); `NodeCreateDialog` đầy đủ metadata + MD template; node title↔lesson title sync khi rename; tab Metadata + breadcrumb cho lesson editor; **catalog phía user: BE filter (category/search/AND-tags/status) + `LimitOffsetPagination` + nút Lọc/Reset render mượt**. 185 BE test pass (+9 mới), smoke Playwright verified. Bugs F42/F43/F44. Report: `docs/reports/2026-06-03_learn-admin-tree-catalog-fixes.md`. |
| Slice 3 / Secret reveal (2026-06-01) | Thêm permission `system.config.read_secret` + endpoint `GET /api/admin/config/{key}/reveal/` (admin-only) để đọc value thật của secret config ([_-08]); list/detail vẫn luôn mask `***`. |
...
| Slice 7 quiz play-flow + scoring fix (2026-06-04) | Fixed quiz session jumping straight to finish (consumer filtered `status='published'`; questions default `draft` → empty set) by serving questions per parent Quiz status (no per-question publish). Reworked the point model: `quiz_point` derived = `SUM(question.score)` via `QuizQuestion` signal (read-only API); completion = 100% of questions ever answered correctly; `UserProfile.total_quiz_point` recomputed absolutely from historically-correct answers (idempotent, no double-count, wrong never subtracts). Added per-user play config (`QuizConfig.question_filter` all/unsolved/solved + `immediate_feedback`) surfaced as a `QuizConfigForm` on the quiz detail page and applied in the consumer (filter/shuffle/cap/feedback). Migrations `0011`/`0012` (fields + backfill). Gates: quiz/profile/leaderboard/consumer pytest (64 passed), `tsc --noEmit`, eslint (quiz files) pass. See `docs/reports/2026-06-04_quiz-playflow-scoring-config.md`. |
...
| Quiz explorer parity (2026-06-04) | `docs/reports/2026-06-04_quiz-explorer-parity.md` |
| Quiz play-flow + scoring + config (2026-06-04) | `docs/reports/2026-06-04_quiz-playflow-scoring-config.md` |
...
| Outline sync API + tab | Low | ✅ Completed 2026-06-06: `OutlineService` (stdlib urllib, WAF-safe User-Agent) + endpoints `GET /api/learn/outline/collections/`, `/documents/`, `POST /api/learn/lessons/{id}/outline/`, `/sync-outline/`, `DELETE …/outline/`; server-mediated (Q-LEARN-06), synchronous blocking with 503-preserves-content (Q-LEARN-10); `outline_info` on lesson detail; Outline tab (collection→doc picker, link/sync/unlink, editor-only source link) wired via `useAdminLearnLessonEditor`; i18n en/vi + MSW handlers. Tests: `backend/api/tests/test_outline_sync.py` (13) + live HTTP E2E vs `collab.n3m3s1s.org`. |
...
| 6.8 GitLab sync + attachment files (Phase 1) | Low | ✅ Completed 2026-06-06: `challenge_file` model + `MEDIA_ROOT` storage; attachment upload/list/delete/download (permission-gated `FileResponse`, members→published only); server-mediated `GitlabService` (urllib + `PRIVATE-TOKEN` + custom UA vs WAF, fetch-first/write-after, 503 preserves old content) → `/api/challenge/gitlab/{projects,projects/{id}/files,import}` + `.../sync-gitlab/`; FE `AdminChallengeFilesTab` + `AdminChallengeGitlabTab` + detail download list; config seeded from `.env` (`GITLAB_URL`/`GITLAB_TOKEN`, like Outline); 30 new tests + **manual E2E PASSED against live GitLab** (import/sync/download/role-gating). See `docs/reports/2026-06-06_task6-8-attachment-gitlab.md`. **Phase 2 (instance deploy server) deferred** → `plan/feature-challenge-task6-9-instance-deploy-server-1.md`. |
| 6.9 Instance deployment (socket backend + deploy server) | Medium | ✅ DONE 2026-06-07 (e2e verified on local Docker host): G1 403 `deploy.enabled` gate, G2 `expires_at` from TTL in `start()`, G3 `challenge.deploy.provider` (mock/socket) selection, G4 race→409 / deploy-fail→503; `SocketDeploymentBackend` (stdlib socket, ILS never imports `docker`) deploy/stop/terminate/extend + exception hierarchy; TTL lazy-expiry + `instance/extend` gated by `challenge.instance_extend_threshold_minutes` with FE countdown; `Challenge.deploy_source_ref` (migration 0014) + admin-form prefill. **Flag injection:** prefers random-tail flag, falls back to static non-regex flag (injected verbatim via env `FLAG`). **Admin instances page:** paginated endpoint + detail dialog exposing `flag_value`/flag-template (`AdminChallengeInstanceSerializer`); sidebar "Instances" link. External `deployer/` (TCP listener + docker-py + TTL reaper + own venv/`tests/`, loads `deployer/.env`), contract `docs/integrations/deploy-socket-protocol.md` + `challenge-repo-format.md`, `examples/challenge-template/`. Real sample challenge `challenges/baby-file-reader` pushed to GitLab + image at `registry.n3m3s1s.org/challenges/baby-file-reader:latest`. **E2E PASSED:** deployer run via Docker CLI, instance launched through real ILS path, per-user FLAG injected + recovered via exploit, container reaped. 30 tests green. **Deployer has NO auth → bind private only; registry pull needs Reporter+ token.** See `docs/reports/2026-06-07_task6-9-instance-deploy.md`. |

### Slice 7 — Quiz

| Task | Priority | Notes |
|------|----------|-------|
| QuizNode tree API | Medium | ✅ Completed 2026-04-01 (folder-only MVP); **upgraded 2026-06-04** to full file-explorer parity with Challenge: atomic item create (draft Quiz + node), `explorer`/`explorer/{id}` endpoints, bulk cycle/depth-safe move |
| Quiz category/tag API + assignment | Medium | ✅ Completed 2026-06-04: `/api/quiz/{categories,tags}/*` CRUD + `category_id`/`tag_ids` on quiz detail + tag-AND/category/solved flat-search filters |
| Frontend: Quiz admin explorer + tabs + taxonomy | Low | ✅ Completed 2026-06-04: table file-explorer (`/admin/quizzes`), editor Tabs (Metadata+category/tag \| Questions inline), taxonomy pages; removed `/new` + `/{id}/questions` routes |
| Frontend: Quiz catalog parity | Low | ✅ Completed 2026-06-04: dual-mode explorer/flat-search + sidebar filter (search/category/tag/solved), matching challenge catalog |
...
| Leaderboard discoverability + UX upgrade | Low | ✅ Completed 2026-06-09: added the **Leaderboard** nav link to user navbar + sidebar (`UserLayout` + 3 layout wrappers + `navigation.leaderboard` i18n) so the existing page is reachable; added a `completed` count per board type to the API entry (`UserProfile.*_completed`, `overall` = sum) surfaced as a new column; added a top-3 medal **podium** (page 1 only) and a **Go to my rank** button (jumps to the page containing `my_rank` and smooth-scrolls to the highlighted row). Gates: leaderboard pytest (7 passed incl. new `completed` test), `tsc --noEmit`, eslint clean. See `docs/reports/2026-06-09_leaderboard-nav-podium-completed.md`. |
