...
- Leaderboard discoverability + UX upgrade completed on 2026-06-09 (report `docs/reports/2026-06-09_leaderboard-nav-podium-completed.md`): the Slice 11 leaderboard page (already built) is now reachable — added the **Leaderboard** nav link to `UserLayout` navbar + sidebar (`leaderboardLabel` prop passed via `tNav('leaderboard')` from all 3 layout wrappers; new `navigation.leaderboard` i18n en/vi). API entries gained a `completed` count per board type (`UserProfile.{course,challenge,quiz}_completed`; `overall`=sum; optional serializer field, backward-compatible) shown as a new table column. Added a top-3 medal **podium** (page 1 only) and a **Go to my rank** button (jumps to `ceil(my_rank/pageSize)` page, smooth-scrolls to the highlighted current-user row). Gates: leaderboard pytest 7 passed (new `test_entry_completed_count_matches_board_type`), `tsc --noEmit`, eslint clean.
- Bugfix batch completed on 2026-06-08 (report `docs/reports/2026-06-08_bugfix-batch-markdown-outline-rbac-instance.md`): (1) FE math rendering via new shared `components/ui/markdown-content.tsx` (remark-math + rehype-katex + katex), reused by lesson viewer/admin preview + challenge description (was plain text). (2) Outline image proxy — `OutlineService.rewrite_attachment_urls` (called in BOTH `link_outline` import + `sync_outline`) rewrites `attachments.redirect` URLs → `/api/learn/lessons/{id}/outline-attachment/?id=…`; `download_attachment` streams bytes server-side (token never leaves BE); FE `MarkdownContent` loads proxy images via authenticated axios→blob. (3) Quiz `immediate_feedback=false` no longer leaks is_correct/explanation/correct_answer over WS (consumer sends bare ack). (4) Instance TTL: new `ChallengeService.reap_expired_instances()` calls `instance.terminate()` (notifies deploy backend) with per-instance try/except; `reap_instances` gains `--loop`; admin instance list sweeps on read. (5) Config: deploy provider default `mock`→`socket` + `enabled`→true; `outline.*` group `outline`→`learn` — both via migration `0015_config_outline_learn_and_deploy_socket`. (6) FE instance panel hides `deploy_instance_id` (UI-only, API unchanged) + Extend button always shown with BE-400 detail surfaced (Option 3, since Members can't read system_config). (7) Admin UX: user Detail dialog, removed redundant course Archive button (status dropdown already archives via same service path), shared `components/ui/truncated-cell.tsx`, RBAC permission/role panels redesigned to single searchable multi-select checkbox + diff-based Save. NOTE: "permission denied creating instance" was NOT an RBAC bug — DB confirmed grants+bitmap correct; it was `challenge.deploy.enabled=false` (403 gate). Latent risk: permission bitmap capped at 256 ids (187 now). Targeted tests green (quiz consumer, outline sync, instance/challenge API, system config, socket backend).
...
- Slice 11 Task 11.3 completed on 2026-04-30: frontend leaderboard page is active at `/{locale}/leaderboard` with canonical `/api/stats/leaderboard/` contract, tab switcher, my-rank summary, row highlight, pagination, and MSW alignment. (Navigation entry was out of scope here — added 2026-06-09, see top Status entry.)
