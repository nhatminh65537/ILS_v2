...
import type { CourseCategory, CourseTag } from '@/types/course.types'

type TaxonomyKind = 'category' | 'tag'

type AdminLearnTaxonomyPageClientProps = {
  locale: string
  kind: TaxonomyKind
}

type TaxonomyItem = CourseCategory | CourseTag

export function AdminLearnTaxonomyPageClient({ locale, kind }: AdminLearnTaxonomyPageClientProps) {
  ...
  const emptyKey = isCategory ? 'empty.noCategories' : 'empty.noTags'
