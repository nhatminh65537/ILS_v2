...
type NodeKind = 'folder' | 'challenge'

type AdminChallengeNodeCreateDialogProps = {
  open: boolean
...
export function AdminChallengeNodeCreateDialog({
  open,
  parentLabel,
  isSubmitting,
  onOpenChange,
  onCreate,
}: AdminChallengeNodeCreateDialogProps) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-3">
          <div className="space-y-1.5">
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="challenge">{t('explorer.kindChallenge')}</SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          {form.kind === 'challenge' ? (
            <p className="text-xs text-muted-foreground">{t('explorer.createChallengeHint')}</p>
          ) : null}
        </div>
